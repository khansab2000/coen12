...
//Sends the elements in SP to an array and then sends that array to quicksort. After sorting it returns the array. 
//getElements runtime O(m)
...
//Recieves initial values from getElements and works recursively with partition to sort it
//quickSort runtime O(n^2) worst case
static void quicksort(void **elts,int lo, int hi, int (*compare)()){
	if (lo<hi){
		int ploc=partition(elts, lo, hi, compare);
		quicksort (elts, lo, ploc-1, compare);
		quicksort (elts,ploc+1,hi, compare);
	}
}

//Recieves values from quicksort and divides them up to be sorted. O(n)
static int partition(void **elts, int lo, int hi, int (*compare)()){
	int i, sep;
	void *x, *temp;
	x=elts[hi];
	sep=lo;
